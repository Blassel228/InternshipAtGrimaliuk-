...
async def delete_by_owner(id_: int, current_user = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    return await company_crud.delete_by_owner(id_=id_, db=db, user_id = current_user.id)
